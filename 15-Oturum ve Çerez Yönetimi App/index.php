<?php
    ...
    // Login değil ise login.php sayfasına yönlendir
    if(!isset($_SESSION['login']) || $_SESSION['login'] == false){
        header('Location:login.php'); 
    }

    if(file_exists(!$hakkimda)){
    ...
    <div class="card <?= cook('color') ? cook('color') : 'bg-dark' ?>" style="width: 18rem;">
        ...
        <div class="card-body">
            ...
            <form action="islem.php?islem=hakkimda" method="POST">
                <textarea class="form-control <?= cook('color') ? cook('color') : 'bg-dark' ?> text-white" name="hakkimda" id="" cols="30" rows="10"><?= htmlspecialchars_decode($hakkimda); ?></textarea>
        ...
        <div class="card-footer bg-info d-flex align-items-center justify-content-between">
            <a href="islem.php?islem=change&color=bg-light" class="btn btn-sm btn-light">Light Mod</a>
            <a href="islem.php?islem=change&color=bg-dark" class="btn btn-sm btn-dark">Dark Mod</a>
        </div>
    </div>
</div>
</body>
